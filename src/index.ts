...
async function universalToolFallback(goal: string, targetShapes: string[]): Promise<GoalSeekResult | null> {
  ...
  // Track whether the model actually AUTHORED an answer, or whether we are
  // about to synthesise one from raw tool output because the loop ran out of
  // iterations. Both are legitimate inputs to the reach judge, but only the
  // first is an answer — surfacing a truncated observation dump to an operator
  // under the heading "Answer" would be a new falsehood in place of the old
  // missing one.
  const authoredFinalAnswer = finalText.trim().length > 0;
...
async function runGoalAsPoolWalk(
  ...
  async function llmExtractPointerArgs(shape: string, correction?: string): Promise<Record<string, unknown> | null> {
   ...
   let howToGuidance = "";
    ...
    // self-description: the discovery registry. Without this the LLM guesses a blind curl/docker
    // command that produces EMPTY stdout (observed: "how many vessels registered" -> empty ->
    ...
    if (execField &&
        /\b(how many|number of|count|sum|total|average|mean|largest|biggest|smallest|list|contain|containing)\b/i.test(goal) &&
        /\b(files|source files|lines|\.ts|\.js|repos\/\w+|src\/|directory|folder|codebase)\b/i.test(goal)) {
      executorGuidance += `FILE-SYSTEM AGGREGATE: the cwd is the super-repo; paths are relative like repos/<vessel>/src. Use find+wc/awk/grep recipes:\n- count files: find PATH -name '*.ts' -type f | wc -l\n- total lines: find PATH -name '*.ts' -type f -exec cat {} + | wc -l\n- average lines: find PATH -name '*.ts' -type f -exec wc -l {} + | awk 'NR>1 {sum+=$1; count++} END {if(count>0) print sum/count}'\n- N largest: find PATH -exec wc -l {} + | sort -rn | grep -v total | head -N\n- files containing text T: grep -rl T PATH | wc -l. Use exact repos/<vessel>/<subpath> named in goal. Output ONLY the number or list under ${execField}, no file writes.\n\n`;
    }
...
If one of those sibling shapes is the action that would create what the goal asks for (e.g. a write/create action), respond with ONLY JSON {"shape": "<sibling shape>", "args": { ...flat pointer arg fields extracted from the goal, e.g. path/content }}. If none of them is an appropriate creating action for this goal, respond with {"shape": null}.`;
  ...
  let walkStepIndex = 0;
  // Walk-level trust verdict, surfaced on goalWalkState at terminalization. Set
  // where the mint-honesty gate computes it; read after the reach-verify try.
  let walkGroundedVerdict: boolean | undefined;
    ...
    const synthetic = new Request("http://local/run-goal", {
      ...
      body: JSON.stringify({
        ...
        operator: (body.operator ?? pointer.operator),
            targetTemplateId: (body.targetTemplateId ?? body.target_template_id ?? pointer.targetTemplateId ?? pointer.target_template_id),
